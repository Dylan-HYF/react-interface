...
const AddAppointment = ({ addAppointment, lastId }) => {
  const [toggleForm, setToggleForm] = useReducer(
  ...
  return (
    <div>
      {/* ternary operators can be put inside ${}, never realize it... */}
      <button onClick={setToggleForm} className={`bg-blue-400 text-white px-2 py-3 w-full text-left ${toggleForm ? 'rounded-t-md' : 'rounded-md'}`}>
        ...
        <div className="border-r-2 border-b-2 border-l-2 border-light-blue-500 rounded-b-md pl-4 pr-4 pb-4">
          <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start  sm:pt-5">
            ...
            <div className="mt-1 sm:mt-0 sm:col-span-2">
              <input type="text" name="ownerName" id="ownerName" onChange={e => setFormData({ ...formData, ownerName: e.target.value })}
          ...
          <div className="pt-5">
            <div className="flex justify-end">
              <button onClick={submit} type="submit" className="ml-3 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-400 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-400">
                Submit
              </button>
            </div>
          </div>
